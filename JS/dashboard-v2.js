/* ═══════════════════════════════════════════════════════════════
   UPSC TRACKER — Dashboard v2 (clean · correct data · quick glance)
   ─────────────────────────────────────────────────────────────
   ✅ Fixed data calc (syllabus from SYLLABUS total, goals, revisions)
   ✅ Countdown hero (Prelims + Mains)
   ✅ 6 KPIs: Today, Streak, Week, Month, Total, Sessions
   ✅ Study Time chart (30D bar with target)
...
(function () {
  ...
  /* ═══════════════ SYLLABUS CALC (same as analytics) ═══════════════ */
  ...
  function renderCountdown() {
    ...
    if (!preDate && !mainsDate) {
      wrap.innerHTML = `
        <div class="dashv2-cd-card prelims" style="grid-column:1/-1">
          <div class="dashv2-cd-label">📅 Exam Countdown</div>
    ...
    wrap.innerHTML = `
      <div class="dashv2-cd-card prelims">
        <div class="dashv2-cd-label">🎯 Prelims Countdown</div>
      ...
      <div class="dashv2-cd-card mains">
        <div class="dashv2-cd-label">📝 Mains Countdown</div>
  ...
  function renderKpis() {
    ...
    const kpis = [
      { lbl: 'Today', val: shortDur(todaySec), sub: `${todayPct}% of target`, color: '#14B8A6' },
      { lbl: '🔥 Streak', val: streak + 'd', sub: `Best: ${bestStreak}d`, color: '#EC4899' },
      { lbl: 'This Week', val: shortDur(weekSec), sub: `Target ${settings.weekly_target || 50}h`, color: '#A855F7' },
      {
        lbl: 'This Month',
    ...
    wrap.innerHTML = kpis
      ...
      <div class="dashv2-kpi" style="--c:${k.color}">
        <div class="dashv2-kpi-lbl">${escHtml(k.lbl)}</div>
      ...
      <div class="dashv2-card">
        <div class="dashv2-sec-head">
          ...
          <div style="flex:1;min-width:0">
            <div class="dashv2-sec-title">Study Time</div>
  ...
  function renderCategoryChart() {
    ...
    if (!catEntries.length) {
      return elFrom(`
        <div class="dashv2-card">
          <div class="dashv2-sec-head">
            <div class="dashv2-sec-icon">🎯</div>
            <div style="flex:1"><div class="dashv2-sec-title">Category Distribution</div></div>
          </div>
    ...
    const card = elFrom(`
      <div class="dashv2-card">
        ...
        <div class="dashv2-split">
          ...
          <div id="dashv2CatLegend" class="dashv2-legend"></div>
        </div>
      </div>
    `);
    ...
    setTimeout(() => {
      ...
      const leg = document.getElementById('dashv2CatLegend');
      if (leg) {
  ...
  function renderUpsell() {
    // Admin aur premium users ko ye nahi dikhana
  ...
  function installDashboardV2() {
    window.renderDashboard = function () {
      ...
      // Charts grid: Study Time + Category
