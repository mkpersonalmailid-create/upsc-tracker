...
(function () {
  ...
  async function deleteNotification(id) {
    myNotifications = myNotifications.filter((n) => n.id !== id);
    unreadCount = myNotifications.filter((n) => !n.read).length;
    updateBadge();
  ...
  function openNotificationsModal() {
    ...
    const itemsHTML = myNotifications.length
      ? myNotifications
          .map((n) => {
            ...
            return `
          <div class="notif-item ${n.read ? '' : 'unread'}" data-notif-id="${n.id}" style="--notif-color:${style.color}">
            ...
            <button class="notif-del-btn" data-notif-del="${n.id}" title="Delete" style="background:none;border:none;color:var(--text-3);cursor:pointer;font-size:.85rem;padding:4px 6px;border-radius:6px">✕</button>
          </div>`;
          })
          .join('')
      : `
    ...
    const unreadTotal = myNotifications.filter((n) => !n.read).length;
    ...
    openModal(
      modalShell({
        ...
        body: `
          <div style="max-height:60vh;overflow-y:auto;margin:-4px 0">${itemsHTML}</div>
        `,
        actions: `
        ...
        onMount() {
          ...
          document.querySelectorAll('[data-notif-id]').forEach((el) => {
            el.onclick = (e) => {
              ...
              if (dot) dot.remove();
              // Update subtitle
              const unreadLeft = myNotifications.filter((x) => !x.read).length;
          ...
          // Delete buttons
          document.querySelectorAll('[data-notif-del]').forEach((btn) => {
            btn.onclick = (e) => {
              e.stopPropagation();
          ...
          if (markAllBtn) {
            markAllBtn.onclick = async () => {
              await markAllAsRead();
              openNotificationsModal(); // re-render
            };
          }
        },
      },
  ...
  async function openSendNotificationModal() {
    ...
    // Fetch all users for target selector
    let users = [];
    try {
      ...
      <div class="field">
        ...
        <select id="sendNotifTarget">
          ...
          ${users
            ...
            .map((u) => `<option value="${u.id}">👤 ${escHtml(u.name || u.email || u.id.slice(0, 8))}</option>`)
            .join('')}
        </select>
      </div>
        ...
        onMount() {
          document.getElementById('sendNotifBtn').onclick = async () => {
            ...
            try {
              ...
              const { error } = await supa.from('user_notifications').insert(rows);
              if (error) throw error;

              if (typeof closeModal === 'function') closeModal();
              if (typeof toast === 'function')
                toast(`✅ Sent to ${targetUserIds.length} user${targetUserIds.length > 1 ? 's' : ''}`, 'ok', 3000);
  ...
  function injectAdminSendButton() {
    ...
    // Find the first card header inside admin view (top of the page)
    ...
    btn.onclick = () => openSendNotificationModal();
  ...
  function waitThenStart() {
    const ready =
      typeof state === 'object' && state !== null && document.getElementById('notifBtn');

    if (ready) {
      injectCSS();
      wireBell();

      // Watch for login → load notifications
      const checkUser = setInterval(() => {
        if (state.user && supa) {
          ...
          // Inject admin button when admin view opens
      ...
      attempts++;
      if (attempts > 200) return console.error('[notifications-extras] timeout');
